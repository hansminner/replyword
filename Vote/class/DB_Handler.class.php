...
class DB_Handler {
    ...
    function insert($table, $fileds, $values) {
        return $this->query("insert into db_vote.$table($fileds)VALUES ($values)");
    }
    ...
    function update($table, $fields, $where) {
        return $this->query("update db_vote.$table set $fields WHERE $table.id = " . $where);
    }
}

//在这里实例化就不用在每一个php页面填写实例化了/ tp框架中的应该是因为直接读取配置,一个项目中数据库通用
$db = new DB_Handler('localhost', 'root', '', 'db_vote');